...
#define CATCH_EXCEPTION(text) catch (ddb::IOException &ex) { throw std::runtime_error(std::string(text) + ex.what()); }catch (ddb::RuntimeException &ex) { throw std::runtime_error(std::string(text) + ex.what()); } catch (...) { throw std::runtime_error(std::string(text) + " unknow exception."); }
...
public:
    DBConnectionPoolImpl(const std::string& hostName, int port, int threadNum = 10, const std::string& userId = "", const std::string& password = "",
            ...
            :dbConnectionPool_(hostName, port, threadNum, userId, password,loadBalance,highAvailability,compress,reConnect,python),
...
class SessionImpl {
public:
    SessionImpl(bool enableSSL=false, bool enableASYN=false, int keepAliveTime=7200, bool compress=false, bool enablePickle=true, bool python=false)
        : host_(), port_(-1), userId_(), password_(), encrypted_(true),
            dbConnection_(enableSSL,enableASYN, keepAliveTime, compress, python), nullValuePolicy_([](ddb::VectorSP) {}), subscriber_(nullptr),subscriberPool_(nullptr),keepAliveTime_(keepAliveTime) {
                dbConnection_.enablePickle(enablePickle);
            }
    ...
    void subscribeBatch(string &host,int port, py::object handler,string &tableName,string &actionName,long long offset, bool resub, py::array filter,
        ...
        if (topicThread_.find(topic) != topicThread_.end()) { throw std::runtime_error("subscription " + topic + " already exists"); }
        bool hasStreamDeser = streamDeserializer.get().isNull()==false;
        ddb::MessageBatchHandler ddbHandler = [handler, msgAsTable, this, hasStreamDeser](std::vector<ddb::Message> msg) {
            // handle GIL
            py::gil_scoped_acquire acquire;
            size_t size = msg.size();   
            py::list pyMsg(size);
            if(hasStreamDeser){
                for (size_t i = 0; i < size; ++i) {
                    py::list row=ddb::DdbPythonUtil::toPython(msg[i]);
                    row.append(msg[i].getSymbol());
                    pyMsg[i] = row;
                }
            }else{
                for (size_t i = 0; i < size; ++i) {
                    pyMsg[i] = ddb::DdbPythonUtil::toPython(msg[i]);
                }
            }
            if(msgAsTable && hasStreamDeser == false){
                py::object dataframe = preserved_->pandas_.attr("DataFrame")(pyMsg);
                handler(dataframe);
            }else {
                handler(pyMsg);
            }
    ...
    ~SessionImpl() {
        ...
        for (auto &it : topics) {
            ...
            try {
                ...
                std::cerr<<"<Exception> in session desctruction: ";
            }
        }
        for (auto &it : topicThread_) {
...
void signal_handler_fun(int signum) {
    ...
    if(!SessionImpl::isSigint_) {
        ...
        for (auto &item : SessionImpl::runningMap_) {
            SessionImpl *session = new SessionImpl(false, false, 7200, false, true, false);
...
public:
    AutoFitTableUpsert(const std::string dbUrl, const std::string tableName, SessionImpl & session,
            ...
            : autoFitTableUpsert(dbUrl,tableName,session.getConnection(),ignoreNull,
    ...
    int upsert(py::object table){
        ...
        int insertRows = 0;
        try {
            insertRows = autoFitTableUpsert.upsert(ddb::DdbPythonUtil::toDolphinDB(table));
        } CATCH_EXCEPTION("<Exception> in append: ")
        return insertRows;
    }
private:
    ...
    ddb::AutoFitTableUpsert autoFitTableUpsert;
};
...
PYBIND11_MODULE(dolphindbcpp, m) {
    ...
    py::class_<SessionImpl>(m, "sessionimpl")
        .def(py::init<bool,bool,int,bool,bool,bool>())
